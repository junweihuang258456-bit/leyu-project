...
void blufi_dh_negotiate_data_handler(uint8_t *data, int len, uint8_t **output_data, int *output_len, bool *need_free)
{
    if (data == NULL || len < 3) {
        ...
        // 配网过程中，当接收到无效数据格式时，发送错误信息给app端，报告数据格式错误
    ...
    if (blufi_sec == NULL) {
        ...
        // 配网过程中，当安全模块未初始化时，发送错误信息给app端，报告安全模块初始化失败
    ...
    case SEC_TYPE_DH_PARAM_LEN:
        ...
        if (blufi_sec->dh_param == NULL) {
            ...
            // 配网过程中，当DH参数内存分配失败时，发送错误信息给app端，报告内存分配失败
    ...
    case SEC_TYPE_DH_PARAM_DATA:{
        if (blufi_sec->dh_param == NULL) {
            ...
            // 配网过程中，当DH参数为空时，发送错误信息给app端，报告DH参数错误
        ...
        if (len < (blufi_sec->dh_param_len + 1)) {
            ...
            // 配网过程中，当DH参数长度无效时，发送错误信息给app端，报告DH参数长度错误
            btc_blufi_report_error(ESP_BLUFI_DH_PARAM_ERROR);
            return;
        }
        ...
        if (ret) {
            ...
            // 配网过程中，当DH参数读取失败时，发送错误信息给app端，报告DH参数读取失败
        ...
        if (dhm_len > DH_SELF_PUB_KEY_LEN) {
            ...
            // 配网过程中，当DH密钥长度不支持时，发送错误信息给app端，报告DH密钥长度不支持
            btc_blufi_report_error(ESP_BLUFI_DH_PARAM_ERROR);
            return;
        }
        ...
        if (ret) {
            ...
            // 配网过程中，当生成公钥失败时，发送错误信息给app端，报告生成公钥失败
        ...
        if (ret) {
            ...
            // 配网过程中，当计算共享密钥失败时，发送错误信息给app端，报告计算共享密钥失败
            btc_blufi_report_error(ESP_BLUFI_DH_PARAM_ERROR);
            return;
        }
        ...
        if (ret) {
            ...
            // 配网过程中，当MD5计算失败时，发送错误信息给app端，报告MD5计算失败
