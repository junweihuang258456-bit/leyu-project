...
static void blufi_profile_cb(tBTA_GATTS_EVT event, tBTA_GATTS *p_data)
{
    ...
    case BTA_GATTS_WRITE_EVT: {
        // 处理GATT写入事件 - 这是APP通过蓝牙发送数据的入口点
        // APP通过蓝牙发送的WiFi列表请求等数据会在这里接收
        ...
        if (p_data->req_data.p_data->write_req.handle == blufi_env.handle_char_p2e) {
            // 调用BLUFI接收处理函数，处理APP发送的数据
            // 这里是APP发送WiFi列表请求等数据的关键处理点
...
void esp_blufi_send_encap(void *arg)
{
    ...
    int retry_count = 0;
    const int max_retry_count = 50; // 最大重试次数，避免无限阻塞
...
retry:
    ...
    if (esp_ble_get_cur_sendable_packets_num(BTC_GATT_GET_CONN_ID(blufi_env.conn_id)) > 0) {
        ...
        if (retry_count >= max_retry_count) {
            BTC_TRACE_ERROR("%s 发送队列阻塞超时，放弃发送数据\n", __func__);
            return;
        }
        
        // 动态调整等待时间，避免固定等待导致的效率问题
        int wait_time = (retry_count < 10) ? 5 : 10; // 前10次等待5ms，之后等待10ms
        BTC_TRACE_WARNING("%s 发送队列满，等待%dms后重试 (第%d次)\n", __func__, wait_time, retry_count);
