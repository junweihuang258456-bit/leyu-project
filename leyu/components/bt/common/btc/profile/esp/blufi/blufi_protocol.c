...
{
    // BLUFI协议处理器 - 解析APP发送的请求数据并执行相应操作
    // 根据数据类型和子类型分发到不同的处理逻辑
    ...
    case BLUFI_TYPE_CTRL:
        ...
        case BLUFI_TYPE_CTRL_SUBTYPE_SET_SEC_MODE:
            // 强制保持安全模式为0x0，禁用加密和校验
            // blufi_env.sec_mode = data[0];
            blufi_env.sec_mode = 0x0;
            BTC_TRACE_WARNING("BLUFI安全模式设置被强制禁用，保持为0x0\n");
            break;
        ...
        case BLUFI_TYPE_CTRL_SUBTYPE_GET_WIFI_LIST:
            // 处理APP请求获取WiFi列表的命令
            // 构造消息并发送到BTC任务，最终会触发ESP_BLUFI_EVENT_GET_WIFI_LIST事件
            msg.sig = BTC_SIG_API_CB;
            msg.pid = BTC_PID_BLUFI;
