...
typedef struct atparser_s *at_handle_t;
...
/**
 * @brief Function to handle unsolicited AT responses (e.g., NO CARRIER, RING)
 *
 * @param at_handle AT parser handle
 * @param response unsolicited response string
 *
 * @return true if the response was handled
 * @return false if not handled
 */
typedef bool (*unsolicited_response_handler_t)(at_handle_t at_handle, const char *response);
...
typedef struct {
    int (*pre_send)(void *usr_data);                                      /* pre send callback, called before sending command (Optional) */
    int (*send_cmd)(const char *cmd_str, size_t cmd_len, void *usr_data); /* send command callback (Required) */
    int (*post_send)(bool is_responsed, void *usr_data);                  /* post send callback, called after sending command (Optional) */
    void *usr_data;
...
/**
 * @brief Register unsolicited response handler
 *
 * This function registers a callback that will be called when an unsolicited
 * response is received (e.g., NO CARRIER when remote party hangs up).
 *
 * @param at_handle AT parser handle
 * @param handler Callback function to handle unsolicited responses
 *
 * @return esp_err_t
 *         - ESP_OK on success
 *         - ESP_ERR_INVALID_ARG if at_handle is NULL
