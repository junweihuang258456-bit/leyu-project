...
// Send custom AT command and get response
esp_err_t at_send_custom_command(at_handle_t at_handle, const char *command, char *res_buffer, size_t res_buffer_len)
{
    ESP_RETURN_ON_FALSE(at_handle != NULL, ESP_ERR_INVALID_ARG, TAG, "Invalid handle");
    ESP_RETURN_ON_FALSE(command != NULL, ESP_ERR_INVALID_ARG, TAG, "Invalid command");
    ...
    at_common_string_t common_str = { .command = command, .string = res_buffer, .len = res_buffer_len};
...
static bool _handle_string(at_handle_t at_handle, const char *line)
{
    ...
    if (end) {
        size_t len = end - start - strlen(AT_RESULT_CODE_SUCCESS);
...
esp_err_t at_cmd_get_imei_number(at_handle_t at_handle, char *res_buffer, size_t res_buffer_len)
{
    ...
    ESP_RETURN_ON_FALSE(res_buffer != NULL, ESP_ERR_INVALID_ARG, TAG, "Invalid parameter");
    ESP_RETURN_ON_FALSE(res_buffer_len > 0, ESP_ERR_INVALID_ARG, TAG, "Invalid length");
    at_common_string_t common_str = { .command = "AT+CGSN=1", .string = res_buffer, .len = res_buffer_len};
    return at_get_common_string(at_handle, &common_str);
}
