...
typedef struct atparser_s {
    ...
    unsolicited_response_handler_t unsolicited_handler; /*!< Function to handle unsolicited responses */
...
at_handle_t modem_at_parser_create(const modem_at_config_t *cfg)
{
    ...
    atparser_t *at_hdl = calloc(1, sizeof(atparser_t));
...
esp_err_t modem_at_parser_destroy(at_handle_t at_handle)
{
    ...
    atparser_t *at_hdl = (atparser_t *)at_handle;
...
esp_err_t modem_at_send_command(at_handle_t at_handle, const char *command, uint32_t timeout_ms, handle_line_func_t handle_line, void *ctx)
{
    ...
    ESP_GOTO_ON_FALSE(handle_line != NULL, ESP_ERR_INVALID_STATE, err, TAG, "AT parser io send_cmd is NULL");
...
#endif
    if (at_hdl->io.pre_send) { // execute pre send callback
        at_hdl->io.pre_send(at_hdl->io.usr_data);
    }
    if (at_hdl->io.send_cmd(at_hdl->send_buffer, len, at_hdl->io.usr_data) != ESP_OK) {
        ret = ESP_FAIL;
    }
    ...
    if (at_hdl->io.post_send) { // execute post send callback
        at_hdl->io.post_send(bits & AT_COMMAND_RSP_BIT, at_hdl->io.usr_data);
    }
...
esp_err_t modem_at_write_response_done(at_handle_t at_handle, size_t write_len)
{
    ...
    if (len > 4 && (strcmp(&lines[len - 2], "\r\n") == 0)) {
        if (at_hdl->handle_line == NULL) {
            // No active command, check if this is an unsolicited response
            if (at_hdl->unsolicited_handler != NULL) {
                lines = strstr(lines, "\r\n") + 2; // skip the first echo line if any
                if (at_hdl->unsolicited_handler(at_handle, lines)) {
                    ret = ESP_OK;
                }
            } else {
                ESP_LOGW(TAG, "AT parser handle_line is NULL but receive: \"%s\"", lines);
            }
            // Clear buffer after handling unsolicited response
            at_hdl->received_length = 0;
            at_hdl->recv_buffer[0] = '\0';
...
esp_err_t modem_at_register_unsolicited_handler(at_handle_t at_handle, unsolicited_response_handler_t handler)
{
    ESP_RETURN_ON_FALSE(at_handle != NULL, ESP_ERR_INVALID_ARG, TAG, "at_handle is NULL");
    atparser_t *at_hdl = (atparser_t *)at_handle;
    at_hdl->unsolicited_handler = handler;
    return ESP_OK;
}
