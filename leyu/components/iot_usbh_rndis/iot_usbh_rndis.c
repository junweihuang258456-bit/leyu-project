...
static esp_err_t usbh_rndis_init_msg_request(usbh_rndis_t *rndis)
{
    ...
    if (ret != ESP_OK) {
        ESP_LOGD(TAG, "Failed to read response %s", esp_err_to_name(ret));
        return ret;
    }
...
static esp_err_t usbh_rndis_query_msg_request(usbh_rndis_t *rndis, uint32_t oid, uint32_t query_len, uint8_t *info, uint32_t *info_len)
{
    ...
    int retry_count = 0;
    const int max_retries = 2;  // 最大重试次数
    uint8_t req_type;  // 将req_type移到函数开始处，确保在整个函数中可见
    ...
    for (retry_count = 0; retry_count <= max_retries; retry_count++) {
        req_type = USB_BM_REQUEST_TYPE_TYPE_CLASS | USB_BM_REQUEST_TYPE_RECIP_INTERFACE | USB_BM_REQUEST_TYPE_DIR_OUT;
    ...
    if (ret != ESP_OK) {
        ESP_LOGD(TAG, "Failed to send query message after %d retries", max_retries + 1);
        return ret;
    }
    ...
    if (ret != ESP_OK) {
        ESP_LOGD(TAG, "Failed to read response after %d retries", max_retries + 1);
        return ret;
    }
...
static esp_err_t usbh_rndis_get_connect_status(usbh_rndis_t *rndis)
{
    ...
    if (ret != ESP_OK) {
        ESP_LOGD(TAG, "Failed to query OID_GEN_MEDIA_CONNECT_STATUS");
        return ret;
    }
...
static esp_err_t usbh_rndis_keepalive(usbh_rndis_t *handle)
{
    ...
    if (ret != ESP_OK) {
        ESP_LOGD(TAG, "Failed to send keepalive message after %d retries", max_retries + 1);
        return ret;
    }
    ...
    if (ret != ESP_OK) {
        ESP_LOGD(TAG, "Keepalive response failed after %d retries", max_retries + 1);
        return ret;
    }
...
static esp_err_t usbh_rndis_connect(usbh_rndis_t *handle)
{
    ...
    return ret;
query_errorout:
    ESP_LOGD(TAG, "rndis query iod:%08x error", oid);
    return ret;
}
...
static esp_err_t usbh_rndis_transmit(iot_eth_driver_t *h, uint8_t *buffer, size_t buflen)
{
    ...
    if (rndis->tx_mutex) {
        xSemaphoreTake(rndis->tx_mutex, portMAX_DELAY);
    }

    if (rndis->connect_status == false || rndis->cdc_port == NULL) {
        if (rndis->tx_mutex) {
            xSemaphoreGive(rndis->tx_mutex);
        }
    ...
    if (hdr == NULL) {
        ...
        ESP_LOGE(TAG, "Failed to allocate memory for RNDIS data packet");
    ...
    ret = usbh_cdc_write_bytes(rndis->cdc_port, (uint8_t *)hdr, len, pdMS_TO_TICKS(500));
    if (ret != ESP_OK) {
        ESP_LOGD(TAG, "Failed to write data to RNDIS");
    }
    ...
    while (1) {
        if (last_data_len == 0) {
            ...
            if (pmsg.MessageType != REMOTE_NDIS_PACKET_MSG) {
                ESP_LOGD(TAG, "Unknown rndis packet message type: 0x%lx, msglength: %lu, data offset: %lu, data length: %lu, rx_length: %d",
...
static void _usbh_rndis_conn_cb(usbh_cdc_handle_t cdc_handle, void *arg, int matched_intf_num)
{
    usbh_rndis_t *rndis = (usbh_rndis_t *)arg;
    usb_device_handle_t usb_dev = NULL;
    usbh_cdc_get_dev_handle(cdc_handle, &usb_dev);
    ...
    ESP_RETURN_VOID_ON_ERROR(usbh_cdc_port_open(cdc_handle, &cdc_port_config, 0, &rndis->cdc_port), TAG, "Failed to open CDC port");
...
static void _usbh_rndis_task(void *arg)
{
    esp_err_t ret = ESP_OK;
    usbh_rndis_t *rndis = (usbh_rndis_t *)arg;
    uint32_t no_event_count = 0;  // 记录无事件发生的次数
    const uint32_t max_no_event_count = 12;  // 最大无事件次数，12 * 5秒 = 60秒
    const uint32_t keepalive_interval = 6;  // 保活间隔，6 * 5秒 = 30秒
    uint32_t keepalive_counter = 0;  // 保活计数器
    uint32_t last_query_fail_count = 0;  // 记录连续查询失败的次数
    const uint32_t max_query_fail_count = 10;  // 最大连续查询失败次数，增加到10次
    uint32_t total_fail_count = 0;  // 记录总失败次数
    const uint32_t warning_interval = 50;  // 每失败50次才打印一次警告
    ...
    while (1) {
        ...
        if (events == 0 && rndis->cdc_port != NULL) {
            ...
            // 只有在达到最大无事件次数时才进行连接状态查询
            if (no_event_count >= max_no_event_count) {
                ESP_LOGD(TAG, "No events for %"PRIu32" seconds, checking connection status", no_event_count * 5);
                ret = usbh_rndis_get_connect_status(rndis);
                if (ret != ESP_OK) {
                    last_query_fail_count++;
                    total_fail_count++;
                    
                    // 只有在连续失败次数达到阈值时才打印警告日志
                    if (last_query_fail_count >= max_query_fail_count) {
                        // 检查是否达到警告间隔
                        if (total_fail_count % warning_interval == 0) {
                            ESP_LOGW(TAG, "Failed to get connect status (%"PRIu32" consecutive failures, %"PRIu32" total failures)", 
                                    last_query_fail_count, total_fail_count);
                        }
                        // 如果连续失败次数太多，减少查询频率
                        no_event_count = max_no_event_count / 2;
                    }
                } else {
                    // 查询成功，重置失败计数器
                    if (last_query_fail_count > 0) {
                        ESP_LOGD(TAG, "Connection status check successful after %"PRIu32" consecutive failures", last_query_fail_count);
                    }
                    last_query_fail_count = 0;
                    no_event_count = 0;  // 重置无事件计数器
                }
            }
            
            // 保活机制，但减少频率
            if (keepalive_counter >= keepalive_interval && rndis->connect_status) {
                ESP_LOGD(TAG, "Sending keepalive after %"PRIu32" seconds", keepalive_counter * 5);
                ret = usbh_rndis_keepalive(rndis);
                if (ret != ESP_OK) {
                    ESP_LOGD(TAG, "Keepalive failed: %s", esp_err_to_name(ret));
                }
                keepalive_counter = 0;  // 重置保活计数器
            }
        }
    }
...
static esp_err_t usbh_rndis_init(iot_eth_driver_t *driver)
{
    ...
    rndis->tx_mutex = xSemaphoreCreateMutex();
    if (rndis->tx_mutex == NULL) {
    ...
    usbh_cdc_device_config_t dev_config = {
        .dev_match_id_list = rndis->config.match_id_list,
        .cbs = {
...
static esp_err_t usbh_rndis_deinit(iot_eth_driver_t *driver)
{
    ...
    ret = usbh_cdc_delete(rndis->cdc_dev);
    ESP_RETURN_ON_ERROR(ret, TAG, "Failed to delete CDC handle");

    vEventGroupDelete(rndis->event_group);
    if (rndis->tx_mutex) {
        vSemaphoreDelete(rndis->tx_mutex);
    }
    free(rndis);
    return ESP_OK;
}
...
usbh_cdc_handle_t usb_rndis_get_cdc_handle(const iot_eth_driver_t *rndis_drv)
{
