...
* Add API `usbh_cdc_get_dev_handle`, `usbh_cdc_port_get_cdc_handle`, `usbh_cdc_port_get_intf_desc`

### Break Changes:

* The `usbh_cdc_device_config_t` structure is modified to adapt to the new API.
* Remove API `usbh_cdc_register_new_dev_cb()` and `usbh_cdc_unregister_new_dev_cb()`.
* The API `usbh_cdc_get_state` replaced by `usbh_cdc_is_connected`.
