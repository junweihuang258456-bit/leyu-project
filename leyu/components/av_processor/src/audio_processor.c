...
static inline esp_opus_enc_frame_duration_t
convert_opus_dec_frame_duration(int frame_duration) {
...
static inline esp_opus_enc_frame_duration_t
convert_opus_enc_frame_duration(int frame_duration) {
  switch (frame_duration) {
  case 2:
...
static int audio_render_write_hdlr(uint8_t *pcm_data, uint32_t len, void *ctx) {
  if ((audio_playback.state != AUDIO_RUN_STATE_PLAYING &&
    ...
    ESP_LOGD(TAG, "Audio playback and feeder are not playing, skip write");
    return ESP_OK;
  }
...
static int playback_out_data_callback(uint8_t *data, int data_size, void *ctx) {
  if (audio_playback.state != AUDIO_RUN_STATE_PLAYING ||
      audio_prompt.state == AUDIO_RUN_STATE_PLAYING) {
    ESP_LOGW(TAG, "Audio playback is not playing, skip write");
    return ESP_OK;
  }
  ESP_LOGD(TAG, "Audio playback out data callback, data size: %d", data_size);
  if (audio_manager.enable_mixer) {
    esp_audio_render_stream_write(
        audio_manager.stream[AUDIO_RENDER_STREAM_PLAYBACK], data, data_size);
  } else {
...
static int playback_event_callback(esp_asp_event_pkt_t *event, void *ctx) {
  if (event->type == ESP_ASP_EVENT_TYPE_MUSIC_INFO) {
    ...
    if (audio_playback.config.event_cb) {
      audio_player_state_t state = AUDIO_PLAYER_STATE_IDLE;
      if (st == ESP_ASP_STATE_STOPPED) {
        state = AUDIO_PLAYER_STATE_STOPPED;
      } else if (st == ESP_ASP_STATE_FINISHED) {
        state = AUDIO_PLAYER_STATE_PLAYING;
      } else if (st == ESP_ASP_STATE_ERROR) {
        state = AUDIO_PLAYER_STATE_ERROR;
      } else if (st == ESP_ASP_STATE_RUNNING) {
        state = AUDIO_PLAYER_STATE_PLAYING;
      } else if (st == ESP_ASP_STATE_PAUSED) {
        state = AUDIO_PLAYER_STATE_PAUSED;
      }
      audio_playback.config.event_cb(state, audio_playback.config.event_cb_ctx);
    }
  }
  return ESP_OK;
}
...
static int prompt_event_callback(esp_asp_event_pkt_t *event, void *ctx) {
  if (event->type == ESP_ASP_EVENT_TYPE_MUSIC_INFO) {
    ...
    esp_asp_state_t st = 0;
    memcpy(&st, event->payload, event->payload_size);
    ESP_LOGI(TAG, "prompt_event_callback: Get State, %d,%s", st,
             esp_audio_simple_player_state_to_str(st));
    if (((st == ESP_ASP_STATE_STOPPED) || (st == ESP_ASP_STATE_FINISHED) ||
         (st == ESP_ASP_STATE_ERROR))) {
...
static esp_err_t recorder_pipeline_event(esp_gmf_event_pkt_t *event,
                                         void *ctx) {
  ESP_LOGD(TAG,
           "CB: RECV Pipeline EVT: el:%s-%p, type:%d, sub:%s, payload:%p, "
           "size:%d,%p",
...
static int recorder_outport_acquire_write(void *handle,
                                          esp_gmf_data_bus_block_t *blk,
                                          int wanted_size, int block_ticks) {
  int res = esp_gmf_fifo_acquire_write(audio_recorder.fifo, blk, wanted_size,
                                       block_ticks);
  if (res != ESP_GMF_ERR_OK && res != ESP_GMF_IO_ABORT) {
    ESP_LOGW(TAG, "Failed to acquire write (%d)", res);
  }
  return res;
}

static int recorder_outport_release_write(void *handle,
                                          esp_gmf_data_bus_block_t *blk,
                                          int block_ticks) {
  int res = esp_gmf_fifo_release_write(audio_recorder.fifo, blk, block_ticks);
  if (res != ESP_GMF_ERR_OK && res != ESP_GMF_IO_ABORT) {
    ESP_LOGW(TAG, "Failed to release write (%d)", res);
  }
  return res;
}

static int recorder_inport_acquire_read(void *handle,
                                        esp_gmf_data_bus_block_t *blk,
                                        int wanted_size, int block_ticks) {
  esp_codec_dev_read(audio_manager.config.rec_dev, blk->buf, wanted_size);
  blk->valid_size = wanted_size;
  if (audio_recorder.config.input_cb) {
    audio_recorder.config.input_cb(blk->buf, blk->valid_size,
                                   audio_recorder.config.input_ctx);
  }
...
static int recorder_inport_release_read(void *handle,
                                        esp_gmf_data_bus_block_t *blk,
                                        int block_ticks) {
  return ESP_GMF_IO_OK;
}

static void esp_gmf_afe_event_cb(esp_gmf_obj_handle_t obj,
                                 esp_gmf_afe_evt_t *event, void *user_data) {
  if (audio_recorder.config.recorder_event_cb) {
    audio_recorder.config.recorder_event_cb((void *)event,
                                            audio_recorder.config.recorder_ctx);
  }
  ...
  case ESP_GMF_AFE_EVT_WAKEUP_START: {
    ...
    ESP_LOGI(TAG, "WAKEUP_START [%d : %d]", info->wake_word_index,
             info->wakenet_model_index);
    break;
  }
  ...
  default: {
    ...
    ESP_LOGW(TAG, "Command %d, phrase_id %d, prob %f, str: %s", event->type,
             info->phrase_id, info->prob, info->str);
    break;
  }
  }
}
...
static esp_err_t recorder_setup_afe_config(void) {
  ...
  srmodel_list_t *models =
      esp_srmodel_init(cfg && cfg->model ? cfg->model : "model");
  if (!models) {
    if (cfg && cfg->ai_mode_wakeup) {
      ESP_LOGE(TAG, "Failed to initialize SR model. When ai_mode_wakeup is "
                    "enabled, model is must be set");
      return ESP_FAIL;
    }
    ESP_LOGW(TAG, "No voice wakeup model found");
  }
  audio_recorder.afe_cfg =
      afe_config_init(audio_manager.config.mic_layout, models, AFE_TYPE_SR,
                      AFE_MODE_LOW_COST); // AFE_TYPE_SR,VC
  ...
  if (audio_recorder.afe_cfg->vad_init) {
    ...
    audio_recorder.afe_cfg->vad_min_speech_ms =
        cfg ? cfg->vad_min_speech_ms : 64;
    audio_recorder.afe_cfg->vad_min_noise_ms =
        cfg ? cfg->vad_min_noise_ms : 1000;
  }
  ...
  esp_gmf_afe_manager_cfg_t afe_manager_cfg = DEFAULT_GMF_AFE_MANAGER_CFG(
      audio_recorder.afe_cfg, NULL, NULL, NULL, NULL);
  if (audio_recorder.config.afe_feed_task_config.task_stack > 0) {
    afe_manager_cfg.feed_task_setting.stack_size =
        audio_recorder.config.afe_feed_task_config.task_stack;
  }
  if (audio_recorder.config.afe_feed_task_config.task_prio > 0) {
    afe_manager_cfg.feed_task_setting.prio =
        audio_recorder.config.afe_feed_task_config.task_prio;
  }
  if (audio_recorder.config.afe_feed_task_config.task_core >= 0) {
    afe_manager_cfg.feed_task_setting.core =
        audio_recorder.config.afe_feed_task_config.task_core;
  }
  if (audio_recorder.config.afe_fetch_task_config.task_stack > 0) {
    afe_manager_cfg.fetch_task_setting.stack_size =
        audio_recorder.config.afe_fetch_task_config.task_stack;
  }
  if (audio_recorder.config.afe_fetch_task_config.task_prio > 0) {
    afe_manager_cfg.fetch_task_setting.prio =
        audio_recorder.config.afe_fetch_task_config.task_prio;
  }
  if (audio_recorder.config.afe_fetch_task_config.task_core >= 0) {
    afe_manager_cfg.fetch_task_setting.core =
        audio_recorder.config.afe_fetch_task_config.task_core;
  }
  esp_err_t ret =
  ...
  esp_gmf_afe_cfg_t gmf_afe_cfg = DEFAULT_GMF_AFE_CFG(
      audio_recorder.afe_manager, esp_gmf_afe_event_cb, NULL, models);
  ...
  ESP_GMF_RET_ON_NOT_OK(
      TAG, ret, { goto _quit; }, "Failed to initialize AFE element");
...
static void recorder_get_pipeline_config(
    const av_processor_encoder_config_t *encoder_cfg, const char ***elements,
    int *element_count, const char **input_name, const char **output_name) {
  ...
  default:
    static const char *pipeline_pcm[] = {"ai_afe", "aud_rate_cvt", "aud_ch_cvt",
                                         "aud_bit_cvt", "aud_enc"};
...
static esp_err_t recorder_setup_pipeline_ports(const char *input_name,
                                               const char *output_name) {
  esp_gmf_port_handle_t outport = NEW_ESP_GMF_PORT_OUT_BLOCK(
      recorder_outport_acquire_write, recorder_outport_release_write, NULL,
      NULL, AUDIO_BUFFER_SIZE, portMAX_DELAY);

  esp_err_t ret = esp_gmf_pipeline_reg_el_port(audio_recorder.pipe, output_name,
                                               ESP_GMF_IO_DIR_WRITER, outport);
  ESP_GMF_RET_ON_NOT_OK(
      TAG, ret, { return ESP_FAIL; }, "Failed to register output port");

  esp_gmf_port_handle_t inport = NEW_ESP_GMF_PORT_IN_BYTE(
      recorder_inport_acquire_read, recorder_inport_release_read, NULL, NULL,
      AUDIO_BUFFER_SIZE, portMAX_DELAY);

  ret = esp_gmf_pipeline_reg_el_port(audio_recorder.pipe, input_name,
                                     ESP_GMF_IO_DIR_READER, inport);
  ESP_GMF_RET_ON_NOT_OK(
      TAG, ret, { return ESP_FAIL; }, "Failed to register input port");
...
static esp_err_t recorder_setup_task_and_run(void) {
  ...
  ESP_LOGI(TAG,
           "创建录音任务前内存状态 - 堆内存: %" PRIu32 " 字节, PSRAM: %" PRIu32
           " 字节",
           (uint32_t)free_heap, (uint32_t)free_psram);

  esp_gmf_task_cfg_t cfg = DEFAULT_ESP_GMF_TASK_CONFIG();
  // 优化：使用更小的栈大小，优先使用PSRAM
  if (audio_recorder.config.recorder_task_config.task_stack > 0) {
    ...
    // 限制最大栈大小，避免内存耗尽
    if (cfg.thread.stack > (4096 * 8)) {
      ESP_LOGW(TAG, "Recorder task stack size too large, limiting to 32k");
      cfg.thread.stack = 4096 * 8;
    }
    if (audio_recorder.config.encoder_cfg.format ==
            AV_PROCESSOR_FORMAT_ID_OPUS &&
        cfg.thread.stack < (4096 * 8)) {
      ESP_LOGW(TAG, "Recorder OPUS encoder requires larger stack size, using "
                    "8k stack");
      cfg.thread.stack = 4096 * 8;
    }
  } else {
    // 默认使用较小的栈大小
    cfg.thread.stack = 4096 * 6;
  }
  ...
  ESP_LOGI(TAG, "创建录音任务 - 栈大小: %d 字节, 使用外部RAM: %s",
           cfg.thread.stack, cfg.thread.stack_in_ext ? "是" : "否");
...
static esp_err_t recorder_configure_all_encoders(void) {
  ...
  const av_processor_encoder_config_t *encoder_cfg =
      &audio_recorder.config.encoder_cfg;
  ...
  case AV_PROCESSOR_FORMAT_ID_G711U:
    ...
    if (encoder_cfg->params.g711.audio_info.channels != 1 ||
        encoder_cfg->params.g711.audio_info.sample_rate != 8000) {
      ESP_LOGW(TAG, "G711 encoder only supports 1 channel and 8000 sample "
                    "rate, using default config");
    }
    ...
    g711_enc_cfg.frame_duration =
        encoder_cfg->params.g711.audio_info.frame_duration;
    esp_audio_enc_config_t g711_cfg = {
        .type = encoder_cfg->format == AV_PROCESSOR_FORMAT_ID_G711A
    ...
    ESP_GMF_RET_ON_NOT_OK(
        TAG, ret, { return ESP_FAIL; }, "Failed to configure G711 encoder");
    esp_gmf_obj_handle_t rate_cvt = NULL;
    esp_gmf_pipeline_get_el_by_name(audio_recorder.pipe, "aud_rate_cvt",
                                    &rate_cvt);
    if (rate_cvt) {
      ret = esp_gmf_rate_cvt_set_dest_rate(rate_cvt, 8000);
      ESP_GMF_RET_ON_NOT_OK(
          TAG, ret, { return ESP_FAIL; },
          "Failed to set G711 rate converter destination rate");
    }
    ...
    ret = esp_gmf_pipeline_report_info(audio_recorder.pipe, ESP_GMF_INFO_SOUND,
                                       &req_info, sizeof(req_info));
    ESP_GMF_RET_ON_NOT_OK(
        TAG, ret, { return ESP_FAIL; },
        "Failed to report sound info for G711 encoder");
    break;
  ...
  case AV_PROCESSOR_FORMAT_ID_OPUS:
    ...
    opus_enc_cfg.frame_duration = convert_opus_enc_frame_duration(
        encoder_cfg->params.opus.audio_info.frame_duration);
    ...
    ESP_GMF_RET_ON_NOT_OK(
        TAG, ret, { return ESP_FAIL; }, "Failed to configure OPUS encoder");
    esp_gmf_obj_handle_t rate_cvt_opus = NULL;
    ret = esp_gmf_pipeline_get_el_by_name(audio_recorder.pipe, "aud_rate_cvt",
                                          &rate_cvt_opus);
    if (ret == ESP_OK && rate_cvt_opus) {
      ret = esp_gmf_rate_cvt_set_dest_rate(rate_cvt_opus, 16000);
      ESP_GMF_RET_ON_NOT_OK(
          TAG, ret, { return ESP_FAIL; },
          "Failed to set OPUS rate converter destination rate");
    }
    ...
    ESP_GMF_RET_ON_NOT_OK(
        TAG, ret, { return ESP_FAIL; },
        "Failed to report sound info for OPUS encoder");
    break;
  case AV_PROCESSOR_FORMAT_ID_PCM:
  default:
    ...
    esp_gmf_pipeline_get_el_by_name(audio_recorder.pipe, "aud_rate_cvt",
                                    &rate_cvt_pcm);
    if (rate_cvt_pcm) {
      ret = esp_gmf_rate_cvt_set_dest_rate(
          rate_cvt_pcm, encoder_cfg->params.pcm.audio_info.sample_rate);
      ESP_GMF_RET_ON_NOT_OK(
          TAG, ret, { return ESP_FAIL; },
          "Failed to set PCM rate converter destination rate");
    }
    esp_gmf_element_handle_t ch_cvt = NULL;
    esp_gmf_pipeline_get_el_by_name(audio_recorder.pipe, "aud_ch_cvt", &ch_cvt);
    if (ch_cvt) {
      ret = esp_gmf_ch_cvt_set_dest_channel(
          ch_cvt, encoder_cfg->params.pcm.audio_info.channels);
      ESP_GMF_RET_ON_NOT_OK(
          TAG, ret, { return ESP_FAIL; },
          "Failed to set PCM channel converter channels");
    }
    esp_gmf_element_handle_t bit_cvt = NULL;
    esp_gmf_pipeline_get_el_by_name(audio_recorder.pipe, "aud_bit_cvt",
                                    &bit_cvt);
    if (bit_cvt) {
      ret = esp_gmf_bit_cvt_set_dest_bits(
          bit_cvt, encoder_cfg->params.pcm.audio_info.sample_bits);
      ESP_GMF_RET_ON_NOT_OK(
          TAG, ret, { return ESP_FAIL; },
          "Failed to set PCM bit converter bits");
    }
    ...
    pcm_enc_cfg.bits_per_sample =
        encoder_cfg->params.pcm.audio_info.sample_bits;
    pcm_enc_cfg.frame_duration =
        encoder_cfg->params.pcm.audio_info.frame_duration;
    ...
    ESP_GMF_RET_ON_NOT_OK(
        TAG, ret, { return ESP_FAIL; }, "Failed to configure PCM encoder");
    ...
    ESP_GMF_RET_ON_NOT_OK(
        TAG, ret, { return ESP_FAIL; },
        "Failed to report sound info for PCM encoder");
    break;
  }
...
static int feeder_outport_acquire_write(void *handle,
                                        esp_gmf_data_bus_block_t *load,
                                        int wanted_size, int block_ticks) {
  return wanted_size;
}

static int feeder_outport_release_write(void *handle,
                                        esp_gmf_data_bus_block_t *blk,
                                        int block_ticks) {
  if (audio_manager.enable_mixer) {
    esp_audio_render_stream_write(
        audio_manager.stream[AUDIO_RENDER_STREAM_FEEDER], blk->buf,
        blk->valid_size);
  } else {
    esp_codec_dev_write(audio_manager.config.play_dev, blk->buf,
                        blk->valid_size);
  }
  ESP_LOGD(TAG, "Feeder outport release write, data size: %d", blk->valid_size);
  return blk->valid_size;
}

static int feeder_inport_acquire_read(void *handle,
                                      esp_gmf_data_bus_block_t *blk,
                                      int wanted_size, int block_ticks) {
...
static int feeder_inport_release_read(void *handle,
                                      esp_gmf_data_bus_block_t *blk,
                                      int block_ticks) {
  return blk->valid_size;
}
...
static esp_err_t feeder_configure_all_decoders(void) {
  ...
  esp_err_t ret = ESP_OK;
  const av_processor_decoder_config_t *decoder_cfg =
      &audio_feeder.config.decoder_cfg;
  esp_gmf_info_sound_t info = {0};
  ...
  case AV_PROCESSOR_FORMAT_ID_PCM:
    ...
    ESP_GMF_RET_ON_NOT_OK(
        TAG, ret, { return ESP_FAIL; }, "Failed to configure PCM decoder");
    break;

  case AV_PROCESSOR_FORMAT_ID_OPUS:
    ESP_LOGI(TAG, "Configuring OPUS decoder");
    esp_opus_dec_cfg_t opus_dec_cfg = ESP_OPUS_DEC_CONFIG_DEFAULT();
    opus_dec_cfg.frame_duration = convert_opus_dec_frame_duration(
        decoder_cfg->params.opus.audio_info.frame_duration);
    ...
    ESP_GMF_RET_ON_NOT_OK(
        TAG, ret, { return ESP_FAIL; }, "Failed to configure OPUS decoder");
  ...
  case AV_PROCESSOR_FORMAT_ID_G711A:
    ...
    ret = esp_gmf_audio_dec_reconfig_by_sound_info(dec_el, &info);
    ESP_GMF_RET_ON_NOT_OK(
        TAG, ret, { return ESP_FAIL; }, "Failed to configure G711A decoder");
    break;
  ...
  case AV_PROCESSOR_FORMAT_ID_G711U:
    ...
    ret = esp_gmf_audio_dec_reconfig_by_sound_info(dec_el, &info);
    ESP_GMF_RET_ON_NOT_OK(
        TAG, ret, { return ESP_FAIL; }, "Failed to configure G711U decoder");
    break;

  default:
    ESP_LOGE(TAG, "Unsupported decoder format: " AV_PROCESSOR_FOURCC_FMT,
             AV_PROCESSOR_FOURCC_ARGS(decoder_cfg->format));
    return ESP_ERR_NOT_SUPPORTED;
  }

  esp_gmf_pipeline_report_info(audio_feeder.pipe, ESP_GMF_INFO_SOUND, &info,
                               sizeof(info));

  esp_gmf_element_handle_t rate_cvt_el = NULL;
  esp_gmf_pipeline_get_el_by_name(audio_feeder.pipe, "aud_rate_cvt",
                                  &rate_cvt_el);
  if (rate_cvt_el) {
    esp_gmf_rate_cvt_set_dest_rate(rate_cvt_el,
                                   audio_manager.config.board_sample_rate);
  }
  esp_gmf_element_handle_t ch_cvt_el = NULL;
  esp_gmf_pipeline_get_el_by_name(audio_feeder.pipe, "aud_ch_cvt", &ch_cvt_el);
  if (ch_cvt_el) {
    esp_gmf_ch_cvt_set_dest_channel(ch_cvt_el,
                                    audio_manager.config.board_channels);
  }
  esp_gmf_element_handle_t bit_cvt_el = NULL;
  esp_gmf_pipeline_get_el_by_name(audio_feeder.pipe, "aud_bit_cvt",
                                  &bit_cvt_el);
  if (bit_cvt_el) {
...
static esp_err_t feeder_setup_pipeline_ports(void) {
  ...
  if (audio_feeder.config.decoder_cfg.format == AV_PROCESSOR_FORMAT_ID_PCM) {
    inport = NEW_ESP_GMF_PORT_IN_BYTE(feeder_inport_acquire_read,
                                      feeder_inport_release_read, NULL, NULL,
                                      AUDIO_BUFFER_SIZE, portMAX_DELAY);
  } else {
    inport = NEW_ESP_GMF_PORT_IN_BLOCK(feeder_inport_acquire_read,
                                       feeder_inport_release_read, NULL, NULL,
                                       AUDIO_BUFFER_SIZE, portMAX_DELAY);
  }
  esp_err_t ret = esp_gmf_pipeline_reg_el_port(audio_feeder.pipe, "aud_dec",
                                               ESP_GMF_IO_DIR_READER, inport);
  ESP_GMF_RET_ON_NOT_OK(
      TAG, ret, { return ESP_FAIL; }, "Failed to register playback input port");

  esp_gmf_port_handle_t outport = NEW_ESP_GMF_PORT_OUT_BYTE(
      feeder_outport_acquire_write, feeder_outport_release_write, NULL, NULL,
      AUDIO_BUFFER_SIZE, portMAX_DELAY);

  ret = esp_gmf_pipeline_reg_el_port(audio_feeder.pipe, "aud_rate_cvt",
                                     ESP_GMF_IO_DIR_WRITER, outport);
  ESP_GMF_RET_ON_NOT_OK(
      TAG, ret, { return ESP_FAIL; },
      "Failed to register playback output port");
...
esp_err_t audio_playback_open(audio_playback_config_t *config) {
  ...
  esp_asp_cfg_t cfg = {
      ...
      .task_stack_in_ext =
          audio_playback.config.playback_task_config.task_stack_in_ext,
  };
...
esp_err_t audio_playback_close(void) {
  ...
  if (audio_playback.player) {
    ...
    ESP_GMF_RET_ON_NOT_OK(
        TAG, err, { return ESP_FAIL; },
        "Failed to destroy audio playback player");
    audio_playback.player = NULL;
  }
...
esp_err_t audio_playback_play(const char *url) {
  ...
  if (audio_playback.state == AUDIO_RUN_STATE_PLAYING) {
    ESP_LOGW(TAG,
             "Audio playback is already playing, stopping current playback");
    esp_audio_simple_player_stop(audio_playback.player);
  }
  ...
  ESP_GMF_RET_ON_NOT_OK(
      TAG, err, { return ESP_FAIL; }, "Failed to start playback");

  audio_playback.state = AUDIO_RUN_STATE_PLAYING;
  return ESP_OK;
}

esp_err_t audio_playback_play_to_end(const char *url) {
  if (!url) {
    ESP_LOGE(TAG, "Invalid URL for playback");
    return ESP_FAIL;
  }
  ...
  esp_err_t err =
      esp_audio_simple_player_run_to_end(audio_playback.player, url, NULL);
  ESP_GMF_RET_ON_NOT_OK(
      TAG, err, { return ESP_FAIL; }, "Failed to wait for playback to end");
  audio_playback.state = AUDIO_RUN_STATE_IDLE;
  return ESP_OK;
}
...
esp_err_t audio_playback_pause(void) {
  ...
  ESP_GMF_RET_ON_NOT_OK(
      TAG, err, { return ESP_FAIL; }, "Failed to pause playback");
...
esp_err_t audio_playback_resume(void) {
  ...
  ESP_GMF_RET_ON_NOT_OK(
      TAG, err, { return ESP_FAIL; }, "Failed to resume playback");
  audio_playback.state = AUDIO_RUN_STATE_PLAYING;
...
esp_err_t audio_playback_stop(void) {
  ...
  ESP_GMF_RET_ON_NOT_OK(
      TAG, err, { return ESP_FAIL; }, "Failed to stop playback");
...
esp_err_t audio_prompt_open(audio_prompt_config_t *config) {
  ...
  esp_asp_cfg_t cfg = {
      ...
      .task_stack_in_ext =
          audio_prompt.config.prompt_task_config.task_stack_in_ext,
  };
  ...
  err = esp_audio_simple_player_set_event(audio_prompt.player,
                                          prompt_event_callback, NULL);
  if (err != ESP_OK) {
...
esp_err_t audio_prompt_close(void) {
  ...
  if (audio_prompt.state == AUDIO_RUN_STATE_PLAYING) {
    // NOTE: Normally, not will reach here, because the prompt is played to end
    // automatically.
  ...
  if (audio_prompt.player) {
    ...
    ESP_GMF_RET_ON_NOT_OK(
        TAG, err, { return ESP_FAIL; },
        "Failed to destroy audio prompt player");
    audio_prompt.player = NULL;
  }
...
esp_err_t audio_prompt_play(const char *url) {
  ...
  esp_err_t err =
      esp_audio_simple_player_run_to_end(audio_prompt.player, url, NULL);
  ESP_GMF_RET_ON_NOT_OK(
      TAG, err, { return ESP_FAIL; }, "Failed to start prompt");
  audio_prompt.state = AUDIO_RUN_STATE_IDLE;
...
esp_err_t audio_recorder_open(audio_recorder_config_t *config) {
  ...
  av_audio_recorder_pool_init(&audio_recorder.pool,
                              &audio_recorder.config.encoder_cfg);
  ESP_GMF_POOL_SHOW_ITEMS(audio_recorder.pool);
  err = recorder_setup_afe_config();
  ESP_GMF_RET_ON_NOT_OK(
      TAG, err, { goto cleanup_and_exit; }, "Failed to setup AFE config");

  const char **elements;
  int element_count = 0;
  const char *input_name = NULL, *output_name = NULL;
  recorder_get_pipeline_config(&audio_recorder.config.encoder_cfg, &elements,
                               &element_count, &input_name, &output_name);

  err = esp_gmf_pool_new_pipeline(audio_recorder.pool, NULL, elements,
                                  element_count, NULL, &audio_recorder.pipe);
  ...
  ESP_GMF_RET_ON_NOT_OK(
      TAG, err, { goto cleanup_and_exit; }, "Failed to configure encoders");

  err = recorder_setup_pipeline_ports(input_name, output_name);
  ESP_GMF_RET_ON_NOT_OK(
      TAG, err, { goto cleanup_and_exit; }, "Failed to setup pipeline ports");

  err = recorder_setup_task_and_run();
  ESP_GMF_RET_ON_NOT_OK(
      TAG, err, { goto cleanup_and_exit; }, "Failed to setup task and run");
...
esp_err_t audio_recorder_get_afe_manager_handle(
    esp_gmf_afe_manager_handle_t *afe_manager_handle) {
...
esp_err_t audio_recorder_get_afe_element_handle(
    esp_gmf_element_handle_t *afe_element_handle) {
  if (!afe_element_handle) {
    ESP_LOGE(TAG, "Invalid parameters for recorder get AFE element handle");
    return ESP_FAIL;
  }
  esp_gmf_pipeline_get_el_by_name(audio_recorder.pipe, "ai_afe",
                                  afe_element_handle);
  if (!afe_element_handle) {
    ESP_LOGE(TAG, "Failed to get AFE element handle");
    return ESP_FAIL;
  }
  return ESP_OK;
}
...
esp_err_t audio_recorder_close(void) {
  ...
  // NOTE: Set task priority to 23 is a workaround for the issue that the
  // recorder task is preempted by other tasks.
...
esp_err_t audio_recorder_read_data(uint8_t *data, int data_size) {
  ...
  int ret = esp_gmf_fifo_acquire_read(audio_recorder.fifo, &blk, data_size,
                                      portMAX_DELAY);
  if (ret < 0) {
  ...
  if (bytes_to_copy > data_size) {
    ESP_LOGI(
        TAG,
        "Read data size is larger than expected, valid size: %d, expected: %d",
...
esp_err_t audio_feeder_feed_data(uint8_t *data, int data_size) {
  ...
  if (audio_feeder.config.decoder_cfg.format == AV_PROCESSOR_FORMAT_ID_PCM) {
    int ret = esp_gmf_db_acquire_write(audio_feeder.fifo, &blk, data_size,
                                       portMAX_DELAY);
    if (ret < 0) {
    ...
    ESP_GMF_RET_ON_NOT_OK(
        TAG, ret, { return ESP_FAIL; },
        "Failed to release write to playback FIFO");
  } else {
    int ret = esp_gmf_db_acquire_write(audio_feeder.fifo, &blk, data_size,
                                       portMAX_DELAY);
    if (ret < 0) {
    ...
    int bytes_to_copy =
        (data_size < blk.buf_length) ? data_size : blk.buf_length;
...
esp_err_t audio_feeder_open(audio_feeder_config_t *config) {
  ...
  if (audio_feeder.config.decoder_cfg.format == AV_PROCESSOR_FORMAT_ID_PCM) {
    ...
    ESP_GMF_RET_ON_NOT_OK(
        TAG, err, { return ESP_FAIL; }, "Failed to create playback ringbuffer");
  } else {
    err = esp_gmf_db_new_fifo(AUDIO_FIFO_NUM, 1, &audio_feeder.fifo);
    ESP_GMF_RET_ON_NOT_OK(
        TAG, err, { return ESP_FAIL; }, "Failed to create playback FIFO");
  }
  const char *elements[] = {"aud_dec", "aud_ch_cvt", "aud_bit_cvt",
                            "aud_rate_cvt"};
  av_audio_feeder_pool_init(&audio_feeder.pool,
                            &audio_feeder.config.decoder_cfg);
  ESP_GMF_POOL_SHOW_ITEMS(audio_feeder.pool);
  err = esp_gmf_pool_new_pipeline(audio_feeder.pool, NULL, elements,
                                  sizeof(elements) / sizeof(char *), NULL,
                                  &audio_feeder.pipe);
  ...
  ESP_GMF_RET_ON_NOT_OK(
      TAG, err, { goto cleanup_and_exit; }, "Failed to configure decoders");
  ...
  size_t free_psram = heap_caps_get_free_size(MALLOC_CAP_SPIRAM);
  ESP_LOGI(TAG,
           "创建播放任务前内存状态 - 堆内存: %" PRIu32 " 字节, PSRAM: %" PRIu32
           " 字节",
           (uint32_t)free_heap, (uint32_t)free_psram);
  ...
  if (audio_feeder.config.feeder_task_config.task_stack > 0) {
    ...
    // 限制最大栈大小，避免内存耗尽
    if (cfg.thread.stack > (4096 * 8)) {
      ESP_LOGW(TAG, "Feeder task stack size too large, limiting to 32k");
      cfg.thread.stack = 4096 * 8;
    }
    if (audio_feeder.config.decoder_cfg.format == AV_PROCESSOR_FORMAT_ID_OPUS &&
        cfg.thread.stack < (4096 * 8)) {
      ESP_LOGW(TAG, "Feeder OPUS decoder requires larger stack size, using "
                    "8k stack");
      cfg.thread.stack = 4096 * 8;
    }
  } else {
  ...
  ESP_LOGI(TAG, "创建播放任务 - 栈大小: %d 字节, 使用外部RAM: %s",
           cfg.thread.stack, cfg.thread.stack_in_ext ? "是" : "否");

  err = esp_gmf_task_init(&cfg, &audio_feeder.task);
  ESP_GMF_RET_ON_NOT_OK(
      TAG, err, { goto cleanup_and_exit; }, "Failed to create playback task");

  ESP_LOGI(TAG, "播放任务创建成功");
  free_heap = esp_get_free_heap_size();
  free_psram = heap_caps_get_free_size(MALLOC_CAP_SPIRAM);
  ESP_LOGI(TAG,
           "创建播放任务后内存状态 - 堆内存: %" PRIu32 " 字节, PSRAM: %" PRIu32
           " 字节",
           (uint32_t)free_heap, (uint32_t)free_psram);
...
esp_err_t audio_feeder_run(void) {
  ...
  ESP_GMF_RET_ON_NOT_OK(
      TAG, ret, { return ESP_FAIL; }, "Failed to start playback pipeline");
...
esp_err_t audio_feeder_stop(void) {
  ...
  ESP_GMF_RET_ON_NOT_OK(
      TAG, ret, { return ESP_FAIL; }, "Failed to stop feeder pipeline");
...
esp_err_t audio_processor_mixer_open(void) {
  ...
  ESP_GMF_RET_ON_NOT_OK(
      TAG, ret, { return ret; }, "Failed to create audio render");

  esp_audio_render_stream_get(
      audio_manager.render_handle, ESP_AUDIO_RENDER_FIRST_STREAM,
      &audio_manager.stream[AUDIO_RENDER_STREAM_FEEDER]);
  esp_audio_render_stream_get(
      audio_manager.render_handle, ESP_AUDIO_RENDER_SECOND_STREAM,
      &audio_manager.stream[AUDIO_RENDER_STREAM_PLAYBACK]);

  esp_audio_render_mixer_gain_t feeder_mixer_gain = {0};
  feeder_mixer_gain.initial_gain =
      audio_feeder.config.feeder_mixer_gain.initial_gain;
  feeder_mixer_gain.target_gain =
      audio_feeder.config.feeder_mixer_gain.target_gain;
  feeder_mixer_gain.transition_time =
      audio_feeder.config.feeder_mixer_gain.transition_time;
  esp_audio_render_stream_set_mixer_gain(
      audio_manager.stream[AUDIO_RENDER_STREAM_FEEDER], &feeder_mixer_gain);

  esp_audio_render_mixer_gain_t playback_mixer_gain = {0};
  playback_mixer_gain.initial_gain =
      audio_playback.config.playback_mixer_gain.initial_gain;
  playback_mixer_gain.target_gain =
      audio_playback.config.playback_mixer_gain.target_gain;
  playback_mixer_gain.transition_time =
      audio_playback.config.playback_mixer_gain.transition_time;
  esp_audio_render_stream_set_mixer_gain(
      audio_manager.stream[AUDIO_RENDER_STREAM_PLAYBACK], &playback_mixer_gain);
  ...
  esp_audio_render_set_out_sample_info(audio_manager.render_handle,
                                       &render_sample_info);
  esp_err_t err = esp_audio_render_stream_open(
      audio_manager.stream[AUDIO_RENDER_STREAM_PLAYBACK], &render_sample_info);
  ESP_GMF_RET_ON_NOT_OK(
      TAG, err, { return err; }, "Failed to open audio render stream");
  err = esp_audio_render_stream_open(
      audio_manager.stream[AUDIO_RENDER_STREAM_FEEDER], &render_sample_info);
  ESP_GMF_RET_ON_NOT_OK(
      TAG, err, { return err; }, "Failed to open audio render stream");
...
esp_err_t audio_processor_ramp_control(audio_mixer_focus_mode_t mode) {
  if (!audio_manager.enable_mixer || !audio_manager.render_handle) {
    ESP_LOGE(TAG,
             "Audio render not initialized or mixer not enabled, enable_mixer: "
             "%d, render_handle: %p",
             audio_manager.enable_mixer, audio_manager.render_handle);
  ...
  case AUDIO_MIXER_FOCUS_FEEDER:
    esp_audio_render_stream_set_fade(
        audio_manager.stream[AUDIO_RENDER_STREAM_FEEDER], true);
    esp_audio_render_stream_set_fade(
        audio_manager.stream[AUDIO_RENDER_STREAM_PLAYBACK], false);
    break;
  case AUDIO_MIXER_FOCUS_PLAYBACK:
    esp_audio_render_stream_set_fade(
        audio_manager.stream[AUDIO_RENDER_STREAM_PLAYBACK], true);
    esp_audio_render_stream_set_fade(
        audio_manager.stream[AUDIO_RENDER_STREAM_FEEDER], false);
    break;
