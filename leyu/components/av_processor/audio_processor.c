...
#ifndef  CONHG_LANGUAGE_WAKEUP_MODE
#define CONHG_LANGUAGE_WAKEUP_MODE 1
#endif

#ifndef CONFIG_VOICE_WAKEUP_MODE
#define CONFIG_VOICE_WAKEUP_MODE 1
#endif
...
esp_err_t audio_recorder_open(recorder_event_callback_t cb, void *ctx)
{
    ...
    esp_err_t err = esp_gmf_db_new_ringbuf(1, 32768, &audio_recorder.fifo);
...
esp_err_t audio_recorder_read_data(uint8_t *data, int data_size)
{
    ...
    esp_gmf_db_release_read(audio_recorder.fifo, &_blk, pdMS_TO_TICKS(50));
...
esp_err_t audio_feeder_feed_data(uint8_t *data, int data_size)
{
    ...
    int ret = esp_gmf_db_acquire_write(audio_feeder.fifo, &blk, data_size, pdMS_TO_TICKS(50));
    if (ret < 0) {
        ESP_LOGE(TAG, "Failed to acquire write to playback FIFO (0x%x)", ret);
        return ESP_FAIL;
    }
    ...
    ret = esp_gmf_db_release_write(audio_feeder.fifo, &blk, pdMS_TO_TICKS(50));
...
esp_err_t audio_feeder_open(void)
{
    ...
    esp_err_t err = esp_gmf_db_new_ringbuf(1, 32768, &audio_feeder.fifo);
    ESP_GMF_RET_ON_NOT_OK(TAG, err, {return ESP_FAIL;}, "Failed to create playback FIFO");

    const char *elements[] = {"aud_dec", "aud_ch_cvt", "aud_rate_cvt"};

    err = esp_gmf_pool_new_pipeline(audio_manager.pool, NULL, (const char **)elements,
...
esp_err_t audio_feeder_reconfig_sample_rate(int sample_rate)
{
    if (audio_feeder.state == AUDIO_RUN_STATE_CLOSED) {
        ESP_LOGE(TAG, "Cannot reconfig feeder - not opened");
        return ESP_FAIL;
    }
    
    ESP_LOGI(TAG, "Reconfiguring audio feeder sample rate to %d Hz", sample_rate);
    ...
    if (!dec_el || !rate_cvt_el) {
        ESP_LOGE(TAG, "Failed to get feeder elements");
        return ESP_FAIL;
    }
    ...
    esp_err_t ret = esp_gmf_audio_dec_reconfig_by_sound_info(dec_el, &info);
    ESP_GMF_RET_ON_NOT_OK(TAG, ret, {return ESP_FAIL;}, "Failed to reconfig PCM decoder");
    
    ret = esp_gmf_rate_cvt_set_dest_rate(rate_cvt_el, sample_rate);
    ESP_GMF_RET_ON_NOT_OK(TAG, ret, {return ESP_FAIL;}, "Failed to reconfig rate converter");
    
    ESP_LOGI(TAG, "Audio feeder sample rate reconfigured successfully");
    return ESP_OK;
}
...
esp_err_t audio_mixer_open(void)
{
    ...
    cfg.ctx = NULL;
    cfg.nb_streams = 2;
    cfg.dst_sample_rate = audio_manager.config.board_sample_rate;
    cfg.dst_channel = audio_manager.config.board_channels;
    cfg.dst_bits = audio_manager.config.board_sample_bits;
    cfg.src_sample_rate = 16000;
    cfg.src_channel = 2;
    cfg.src_bits = 16;
    ...
    esp_err_t err = esp_gmf_db_new_ringbuf(1, 32768, &audio_mixer.bus_array[0]);
    if (err != ESP_GMF_ERR_OK || !audio_mixer.bus_array[0]) {
        ESP_LOGE(TAG, "Failed to create audio mixer bus");
        goto cleanup_and_exit;
    }
    err = esp_gmf_db_new_ringbuf(1, 32768, &audio_mixer.bus_array[1]);
