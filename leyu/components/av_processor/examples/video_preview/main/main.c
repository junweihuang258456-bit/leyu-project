...
static const char *TAG = "video_passthru";
...
static void video_capture_decode_callback(void *ctx, int index, video_frame_t *vid_frame)
{
    video_render_feed_frame(render_handle, vid_frame);
}
...
static esp_err_t video_passthru_init(void)
{
    ...
    video_capture_config_t capture_cfg = {
        .capture_frame_cb = video_capture_decode_callback,
