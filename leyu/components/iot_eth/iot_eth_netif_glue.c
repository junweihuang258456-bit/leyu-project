...
static esp_err_t iot_eth_unregister_glue_event(iot_eth_netif_glue_handle_t eth_netif_glue)
{
    ...
    ret = esp_event_handler_unregister(IP_EVENT, IP_EVENT_PPP_GOT_IP, eth_action_got_ip);
    ESP_RETURN_ON_ERROR(ret, TAG, "Fail to unregister eth_action_got_ip (%s)", esp_err_to_name(ret));
...
static esp_err_t iot_eth_register_glue_event(iot_eth_netif_glue_handle_t eth_netif_glue)
{
    ...
    ESP_RETURN_ON_ERROR(ret, TAG, "Fail to register eth_action_handle (%s)", esp_err_to_name(ret));

    ret = esp_event_handler_register(IP_EVENT, IP_EVENT_ETH_GOT_IP, eth_action_got_ip, eth_netif_glue);
    ret = esp_event_handler_register(IP_EVENT, IP_EVENT_PPP_GOT_IP, eth_action_got_ip, eth_netif_glue);
    if (ret != ESP_OK) {
        esp_event_handler_unregister(IP_EVENT, IP_EVENT_ETH_GOT_IP, eth_action_got_ip);
        esp_event_handler_unregister(IP_EVENT, IP_EVENT_PPP_GOT_IP, eth_action_got_ip);
        return ESP_FAIL;
    }
